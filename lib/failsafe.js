...
export function validateBinTable($, { requiredKeyword = "Ness" }) {
  ...
  // ✅ Look for Ness (case-insensitive, allows prefixes like "Week 2 - Ness")
  ...
  if (!foundKeyword) {
    const areas = $("tbody tr").map((i, row) => $(row).find("td").first().text().trim()).get();
    throw new Error(`No row containing "${requiredKeyword}" found. Areas seen: ${areas.join(", ")}`);
  }
}
