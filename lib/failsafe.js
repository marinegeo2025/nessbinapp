...
export function validateBinTable($, { requiredKeyword = "Ness" }) {
  ...
  // ✅ Look for keyword match (case-insensitive, partial match)
  ...
  if (!foundKeyword) {
    throw new Error(`No row containing "${requiredKeyword}" found (area names were: ${
      $("tbody tr").map((i, row) => $(row).find("td").first().text().trim()).get().join(", ")
    })`);
  }
}
